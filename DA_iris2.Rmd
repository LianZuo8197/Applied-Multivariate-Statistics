---
title: "Stat 577: Discriminant Analysis - Iris data"
author: "T Shapla"
date: "11/08/2023"
...
pred.new.obs=predict(lda.iris, new.obs, prior = lda.iris$prior) #predict(model.object, newdata)
#or, pred.new.obs=predict(lda.iris, new.obs, prior = lda.iris$prior)
pred.new.obs
```
...
library(caret)
confusion_matrix <- confusionMatrix(predictions$class, test.data$Species, dnn = c("Predicted Group", "Reference/original Group"))
print(confusion_matrix)
```

## Quadratic Discriminant Analysis

Below we perform the quadratic discriminant analysis for the iris data. In this case, we use qda() function from MASS package. All other coding are same as before. The model accuracy is found to be 0.9666667, which implies the quadratic discriminant function is performing better than the linear discriminant function in predicting group membership of iris flowers. This makes sense since quadratic discriminant function is more suitable and appropriate here due to heterogeneity of covariance matrices of underlying variable across three species.   
...
prediction.qda=predict(qda.iris, test.trs, prior = qda.iris$prior)
mean(prediction.qda$class==test.trs$Species)
```

## Prediction of new observations

Now, we will predict the group membership of same 3 observations described above with measurements on original scale. We will use the quadratic discriminant function based on original un-transformed 'train.data' to predict the species.


```{r}
qda.untrs=MASS::qda(Species~., data=train.data)
Sepal.Length=c(5.1, 4.9, 6.7)
Sepal.Width=c(3.5, 3, 3.3)
Petal.Length=c(1.4,1.4,5.7)
Petal.Width=c(0.2,0.2, 2.5)
new.untr=data.frame(Sepal.Length, Sepal.Width, Petal.Length, Petal.Width)
predict.new.untr.qda=predict(qda.untrs, new.untr, prior = qda.iris$prior)
